#include <nxp/iolpc2148.h>

// LCD Pins
#define rs IO0PIN_bit.P0_8
#define en IO0PIN_bit.P0_11

// LCD Commands
unsigned char commandarray[5] = {0x38, 0x01, 0x06, 0x0c, 0x80};

// Delay
void delay(unsigned int x) {
    unsigned int i;
    for (i = 0; i < x; i++);
}

// LCD Initialization
void lcd_init() {
    int i;
    for (i = 0; i < 5; i++) {
        IO1PIN = commandarray[i] << 16;
        rs = 0;
        en = 1;
        delay(4095);
        en = 0;
    }
}

// LCD Command
void lcd_cmd(unsigned char cmd) {
    IO1PIN = cmd << 16;
    rs = 0;
    en = 1;
    delay(4095);
    en = 0;
}

// LCD Data (print string)
void lcd_print(char *str) {
    while (*str != '\0') {
        IO1PIN = (*str) << 16;
        rs = 1;
        en = 1;
        delay(500);
        en = 0;
        str++;
    }
}

// Integer to string (itoa)
void intToStr(int num, char *str) {
    int i = 0, j, rem;
    char temp[10];
    if (num == 0) {
        str[0] = '0';
        str[1] = '\0';
        return;
    }
    while (num != 0) {
        rem = num % 10;
        temp[i++] = rem + '0';
        num /= 10;
    }
    for (j = 0; j < i; j++) {
        str[j] = temp[i - j - 1];
    }
    str[i] = '\0';
}

// Keypad Scan Function
char get_key() {
    int row_vals[] = {0x000E0000, 0x000D0000, 0x000B0000, 0x00070000};
    char key_map[4][4] = {
        {'1', '2', '3', 'A'},
        {'4', '5', '6', 'B'},
        {'7', '8', '9', 'C'},
        {'*', '0', '=', 'D'}
    };

    int row, col;

    for (row = 0; row < 4; row++) {
        IO0PIN = row_vals[row]; // set one row low at a time
        delay(1000);
        int col_val = IO0PIN & 0x00F00000;
        if (col_val != 0x00F00000) {
            for (col = 0; col < 4; col++) {
                if ((col_val & (1 << (20 + col))) == 0) {
                    while ((IO0PIN & 0x00F00000) != 0x00F00000); // wait for key release
                    return key_map[row][col];
                }
            }
        }
    }
    return 0;
}

void main() {
    int sum = 0;
    char key;
    char buffer[20];

    // LCD + GPIO Setup
    PINSEL0 = 0x00000000;
    PINSEL2 = 0x00000000;
    IO0DIR |= 0x000F0000; // P0.16 to P0.19 as row output
    IO0DIR &= ~0x00F00000; // P0.20 to P0.23 as column input
    IO0DIR_bit.P0_8 = 1;  // RS
    IO0DIR_bit.P0_11 = 1; // EN
    IO1DIR = 0x00FF0000;  // LCD data pins on P1.16–P1.23

    lcd_init();
    lcd_cmd(0x80);
    lcd_print("Enter numbers");

    while (1) {
        key = get_key();
        if (key >= '0' && key <= '9') {
            sum += key - '0'; // Convert char to int
            lcd_cmd(0xC0); // Move to second line
            lcd_print("Added: ");
            lcd_print(&key); // show latest number (will print ASCII char)
        } else if (key == 'D') { // Show result
            lcd_cmd(0x01); // Clear
            lcd_cmd(0x80);
            lcd_print("Total = ");
            intToStr(sum, buffer);
            lcd_print(buffer);
        } else if (key == 'C') { // Clear/reset
            sum = 0;
            lcd_cmd(0x01);
            lcd_cmd(0x80);
            lcd_print("Enter numbers");
        }
        delay(100000); // Debounce delay
    }
}
